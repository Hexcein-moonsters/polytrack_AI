...
training_worker.onmessage = (e) => {
    ...
    if (type == "model_init_done") {
        ...
        if (data.isNewCar) iterationCount = 0; // kinda useless but an extra check isn't excessive
            ...
            iterationCount = localStorage.getItem("AI_PPO_iterationCount") || 0; // reset if not found
            localStorage.setItem("AI_PPO_iterationCount", iterationCount);
        }
        ...
        if (progressInfo) console.log("Training car " + carID + " done in " + trainingTime.toFixed(3) + "s");
        // Now save the model
        ...
        postToWorker({
            messageType: Q_.AI_makeRecordingString,
            inputs: inputs
        });
    } else {
...
// It takes about 360ms to load the model
...
function onWorkerMessage(e) { // simulation worker -> main thread
    if (e.messageType === Q_.DeterminismResult) {
        ...
        if (hasFinished) { // this means the AI has already crossed the finish line but the car hasn't been deleted yet somehow
            ...
            if (progressInfo) console.log("Deleting car " + carID + " as it hasn't finished within the 100s. Current time: " + lastState.frames + " frames.");
...
function createAI_car(carID, trackData) {
    ...
    if (progressInfo) console.log("Starting car " + carID);
...
function AI_controlsrequested_handler(carID, states) { // this func only gets called after 'model_init_done' as otherwise no AI cars could have existed
    ...
    function flattenState(state) {
        const buffer = new ArrayBuffer(4); // 15MB buffer // 150 * 100
        const view = new DataView(buffer);
        let offset = 0;

        // Write state.position.x as float (4 bytes)
        view.setFloat32(offset, performance.now(), true); offset += 8;

        // ... write *every single field* in order (no objects!)

        return buffer;
    }

    // Usage (only do this once per frame, NOT per message):
    //const flatBuffer = flattenState(states[states.length - 1]); // 15MB buffer (done once)
    const flatBuffer = new Float32Array(96); // the length must be the length of agentStates plus 1 mandatory startsAtIndex byte and then the number of extra data. 96 here as 5+91 // Just do startsAtIndex + agentState length
    ...
    const agentState = getAgentState(states);
...
function AI_endOfEpisode_handler(carID, lastState, hasFinished) {
    postToWorker({ messageType: Q_.DeleteCar, carId: carID }); // First delete the car

    const inferenceTime = (performance.now() - inferenceTimePerCar[carID]) / 1000;
    delete inferenceTimePerCar[carID];
    if (progressInfo) console.log("Inference of car " + carID + " took " + inferenceTime.toFixed(3) + "s");
    ...
    // now train
    if (progressInfo) console.log("Training car " + carID);

    training_worker.postMessage({ type: 'train', data: { carID: carID, epochs: 1, batchSize: 32 } });
}
...
function getAgentState(states) {
    ...
    const agentState = [
        ...
        // Positional data
        lastSimState.position.x,
        lastSimState.position.y,
        lastSimState.position.z,
        // Delta position, which is a velocity vector
        secondLastSimState.position.x - lastSimState.position.x,
        secondLastSimState.position.y - lastSimState.position.y,
        secondLastSimState.position.z - lastSimState.position.z,
