...
setShared("postToWorker", (e) => { onmessage({ data: e }) }); // instantly process, by calling i(data)
        ...
        function i(e) {
            switch (e.data.messageType) {
                case Q_.Init:
                    {
                        ...
                        getShared("onCommunicatorReady")(); // call it
                ...
                case Q_.AI_Init:
                    {
                        ...
                        getShared("onCommunicatorReady")("AI_Init"); // call it
                        break;
                    }
                case Q_.AI_fromEnv_updateControls:
                    ...
                    break;
                case Q_.AI_makeRecordingString:
                    !function (e) {
                        ...
                        inputs.forEach((input) => {
                            ...
                            replayMaker.recordFrame(currentFrame, input.inputs);
                        });
                        ...
                        return;
                        postMessage({
                            messageType: Q_.AI_recordingStringResult,
                            carId: e.data.carId,
                            requestID: e.data.requestID,
                            recordingString: n.find(t => t.id == e.data.carId).model.makeRecordingString()
                        })
                    }(e);
            }
        }
                ...
                for (const car of nonPausedCars) {
                    ...
                    if (car.model.hasStarted()) {
                        ...
                        if ( // Execute statements in sync, not parallel, = don't waste compute power on 2nd statement if 1st is false
                            currentTime.numberOfFrames < Sh.maxFrames && // "bh.maxFrames = 5999999;" = checking if sim has lasted less than 99.9 minutes
                            currentTime.lessThan(car.targetSimulationTime) &&
                            !car.isPaused
                        ) {
                            if (currentTime.numberOfFrames == 0) {
                            ...
                            //console.log(currentTime.numberOfFrames, car.model.controls);
                            //console.log(car.model.getSpeedKmh());
                            if (car.userControls != null) {
                                for (; car.userControls.length > 0 && car.userControls[0].frame <= currentTime.numberOfFrames;) { // will loop until the first control is on this frame
                                    //console.log("Removed one userControls at frame: " + currentTime.numberOfFrames);
